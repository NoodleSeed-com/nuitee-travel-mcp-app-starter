...
Live one-way search and same-session fare verification passed again on 2026-08-24, and the owner confirmed repeat verification in ChatGPT. The real 30-minute state test correctly rejected the expired active fare, but the same `0.136.0` server did not complete a fresh state write; that layer remains open for the separately reviewed `0.137.0` update. Two bounded round-trip attempts completed application mapping but returned Nuitee `provider_error`, so round-trip and large-response success also remain open evidence gates. A complete owner-authorized round-trip response had previously measured 4,960,533 decoded bytes, above the former 3 MiB operation ceiling. `@noodleseed/one` 0.116 raised the opt-in per-operation maximum to 6 MiB; this starter applies that ceiling only to flight search and proves the measured response class reaches bounded normalization hermetically. Fare verification keeps the smaller 750,000-byte application limit and no widened connector limit. See [docs/live-smoke-evidence.md](docs/live-smoke-evidence.md) for the sanitized result. Nuitee documents no result limit or pagination contract.
