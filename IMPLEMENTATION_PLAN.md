...
| Decision | Resolution |
| --- | --- |
| Package | Pin `@noodleseed/one` exactly to `0.132.1`; regenerate the lockfile and Agent Kit on every update. |
...
Correction outcome: the reviewed scaffold failure was narrowly corrected by pinning the approved exact package, regenerating the lock, refreshing Agent Kit, and removing the active embedded-assistant block from the default server. The repository now resolves exact `0.132.1`; the generated Codex and Claude instructions identify Agent Kit `0.79.0`, and `noodle agents doctor --json` reports both targets current with no restart required. The corrected baseline requires neither a provider nor assistant-model credential.
